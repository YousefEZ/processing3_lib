...
class container:
    ...
    def widget_key_taken(self, key_name):
        if key_name == "WINDOW" or key_name in (self.__buttons.keys() + self.__labels.keys() + self.__entries.keys()):
            return True
        return False
    

    ## for mouse activity / INPUT
    ...
    def add_button(self, widget_key, button_text, coords, widget_size, left_function=None, left_args=[], right_function=None, right_args=[]):
        if self.widget_key_taken(widget_key):
            raise WidgetNameTakenException
        
        self.__buttons[widget_key] = button(widget_key, button_text, coords, widget_size, left_function,left_args,  right_function, right_args)
        self.__button_func_params[widget_key] = [left_args, right_args]
    ...
    ## The following is to adjust the values inside widgets
